...
class TestSessionResolution:
    ...
    @patch("mob.cli.resolver.api_get", return_value=SESSIONS)
    def test_session_by_name(self, mock_get):
        assert resolve_ref("session", "coder-a1b2c3d4") == "ar-1"

    @patch("mob.cli.resolver.api_get", return_value=SESSIONS)
    def test_session_by_position(self, mock_get):
        assert resolve_ref("session", "2") == "ar-2"

    def test_session_uuid_passthrough(self):
        uuid = "12345678-1234-1234-1234-123456789abc"
        assert resolve_ref("session", uuid) == uuid

    @patch("mob.cli.resolver.api_get", return_value=SESSIONS[:2])
    def test_session_with_agent_filter(self, mock_get):
        result = resolve_ref("session", "1", agent_id="ag-1")
        assert result == "ar-1"
        mock_get.assert_called_with("/sessions", params={"agent_id": "ag-1"})

    @patch("mob.cli.resolver.api_get", return_value=SESSIONS)
    def test_session_name_not_found(self, mock_get):
        with pytest.raises(click.ClickException, match="No session with name 'nonexistent'"):
            resolve_ref("session", "nonexistent")
